<div id="divBookings">
    <div class="form-row mb-3">
        ...
        <div class="form-group col-md-2">
            ...
            <select wire:model="orderBy" class="form-control">
                <option value="bookings_id">ID</option>
    ...
    <div class="d-none d-sm-none d-md-none d-lg-block">
        <table class="table-auto w-full mb-6 table-responsive-sm">
            <tbody>
                ...
                @foreach($data as $row)
                    <tr>
                        <td class="border px-4 py-2">{{ $row->bookings_id }}</td>
                        <td class="border px-4 py-2"><?php echo App\Semesters::term_year($row->semesters); ?></td>
                        <td class="border px-4 py-2">{{ $row->numbers }}</td>
                        <td class="border px-4 py-2">{{ $row->fname }} {{ $row->lname }}</td>
                        <td class="border px-4 py-2">{{ $row->tel }}</td>
                        ...
                        <td class="border px-4 py-2">
                            ...
                            <div class="dropdown-menu text-center">
                                ...
                                <?= Form::close() ?>
                                <div class="dropdown-divider"></div>
                                <button type="button" @if($row->bookSta==0||$row->bookSta==3) disabled @endif data-BookingsId=<?php echo Crypt::encrypt($row->bookings_id); ?> class="btn btn-success btn-sm btn-block btnApprove" ><i class="fas fa-clipboard-check"></i> อนุมัติการจอง</button>
                                <div class="dropdown-divider"></div>
                                <?= Form::open(array('route' => 'admin.history.display.update','style' => 'display:inline;')) ?>
                                    <?= Form::hidden('BookingsId', Crypt::encrypt($row->bookings_id) ); ?> 
    ...
    {{-- Mobile --}}
    <pre class="d-block d-sm-block d-md-block d-lg-none kanin">
        <table class="table-auto w-full mb-6 table-responsive-sm">
            <tbody>
                <tr>
                    <th class="px-4 py-2 fixCol">ID</th>
                    <th class="px-4 py-2">ภาคเรียน</th>
                    <th class="px-4 py-2">ห้อง</th>
                    <th class="px-4 py-2">ชื่อ</th>
                    <th class="px-4 py-2">เบอร์</th>
                ...
                @foreach($data as $row)
                    <tr>
                        <td class="border px-4 py-2 fixCol">{{ $row->bookings_id }}</td>
                        <td class="border px-4 py-2">{{ $row->semesters }}</td>
                        <td class="border px-4 py-2">{{ $row->numbers }}</td>
                        <td class="border px-4 py-2">{{ $row->fname }} {{ $row->lname }}</td>
                        <td class="border px-4 py-2">{{ $row->tel }}</td>
                        <td class="border px-4 py-2">{{ App\Classrooms::Droom($row->days) }}</td>
